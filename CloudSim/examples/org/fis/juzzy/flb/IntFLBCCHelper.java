...
public class IntFLBCCHelper {
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			int vmType = i / (int) Math.ceil((double) vmsNumber / IntFLBCCConstants.VM_TYPES);
			vms.add(new PowerVm(
					i,
					brokerId,
					IntFLBCCConstants.VM_MIPS[vmType],
					IntFLBCCConstants.VM_PES[vmType],
					IntFLBCCConstants.VM_RAM[vmType],
					IntFLBCCConstants.VM_BW,
					IntFLBCCConstants.VM_SIZE,
					1,
					"Xen",
					new CloudletSchedulerDynamicWorkload(IntFLBCCConstants.VM_MIPS[vmType], IntFLBCCConstants.VM_PES[vmType]),
					IntFLBCCConstants.SCHEDULING_INTERVAL));
		}
		return vms;
	}
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			int hostType = i % IntFLBCCConstants.HOST_TYPES;

			List<Pe> peList = new ArrayList<Pe>();
			for (int j = 0; j < IntFLBCCConstants.HOST_PES[hostType]; j++) {
				peList.add(new Pe(j, new PeProvisionerSimple(IntFLBCCConstants.HOST_MIPS[hostType])));
			}

			hostList.add(new PowerHostUtilizationHistory(
					i,
					new RamProvisionerSimple(IntFLBCCConstants.HOST_RAM[hostType]),
					new BwProvisionerSimple(IntFLBCCConstants.HOST_BW),
					IntFLBCCConstants.HOST_STORAGE,
					peList,
					new VmSchedulerTimeSharedOverSubscription(peList),
					IntFLBCCConstants.HOST_POWER[hostType]));
		}
		return hostList;
	}
